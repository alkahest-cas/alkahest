...
#: Tiers generated by the LIOUVILLE construction rather than the small legacy
#: grammars. These go through the verification gate before a row is emitted.
_LIOUVILLE_TIERS = (3, 4)
...
def _make_row(tier: int, nonelementary: bool, rng: random.Random) -> dict:
    ...
    if tier in _LIOUVILLE_TIERS:
        return _liouville_row(tier, rng)

    f_antideriv = random_elementary(pool, tier, rng)
    f_result = ak.diff(f_antideriv, x)
    f = ak.simplify(f_result.value).value
    return _elementary_row(f, f_antideriv, tier)


def _elementary_row(f, integral, tier: int) -> dict:
    return {
        "prompt": [{"role": "user", "content": f"Find ∫ {ak.unicode_str(f)} dx"}],
        "f_str": str(f),
        "F_str": str(integral),
...
def _liouville_row(tier: int, rng: random.Random, max_attempts: int = 8) -> dict:
    """A tier 3/4 row, generated by LIOUVILLE and **verified before it is emitted**.

    A wrong ``(f, F)`` pair poisons the reward, so no unverified pair reaches a
    dataset: :func:`~alkahest.rl.envs.integration.corpus.verify_pair` checks
    ``d/dx F == f`` symbolically and — independently of ``ak.diff`` — by finite
    difference at in-domain points, and a failure is resampled rather than
    emitted. Roughly 4% of draws are discarded, all of them for being
    domain-restricted rather than wrong.
    """
    # Imported lazily: ``corpus`` imports this module's siblings, and
    # ``python -m alkahest.rl.envs.integration.corpus`` warns about a double
    # import if the edge is taken at module scope.
    from alkahest.rl.envs.integration.corpus import verify_pair
    ...
    for _ in range(max_attempts):
        ...
        try:
            ...
            last = exc
            continue
        if verify_pair(pair, pool).ok:
            return _elementary_row(pair.integrand, pair.integral, tier)
    msg = (
        f"could not produce a verified tier-{tier} pair in {max_attempts} attempts"
