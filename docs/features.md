...
- `RootSum` kernel node: first-class symbolic sum over algebraic roots, with differentiation, **numeric evaluation** (`eval_expr`, and therefore the antiderivative gate — the minimal polynomial's roots are found numerically and the body summed over them in complex arithmetic), display (Debug / LaTeX / unicode), persistence (pool format V5), and PyO3 bridge. The evaluation is an `f64` screen and declines rather than guessing: an unreadable minimal polynomial, a root set that fails a backward-error check, or a sum that is not real all return "no value". It is also ill-conditioned in the roots, so above about degree 14 the answer, though correct, can no longer be checked in double precision and the integrator declines it
...
- Special functions beyond the elementary set: `lambert_w`, `digamma`, `trigamma`, `bessel_j0`/`bessel_j1`, the elliptic family (`EllipticK`/`EllipticE`/`EllipticF`/`EllipticPi`, parameter convention `m = k²`), the Fresnel pair `fresnels`/`fresnelc` (normalised π/2 convention — DLMF §7.2(iii); `S(∞) = C(∞) = 1/2`) and the dilogarithm `dilog` (principal branch, cut on `[1, ∞)`; real-valued on `(−∞, 1]` and declining past the cut). All of these now differentiate — `Γ′ = Γψ`, `ψ′ = ψ₁`, `S′ = sin(πx²/2)`, `C′ = cos(πx²/2)`, `Li₂′ = −log(1−x)/x`, and `Π(n; φ | m)` in all three of its arguments — which is what lets the integrator's verification gate check `d/dx F = f` on an antiderivative that carries one. The single deliberate exception is `trigamma`, which declines: the polygamma ladder has no closed-form terminator short of a binary `polygamma(n, x)`, and declining is reported as `E-DIFF-001` rather than answered with a placeholder. Differentiating and *bounding* are separate questions: the elliptic family still has no Taylor-model rule (the validated tier's rules are unary, and `EllipticPi` takes three arguments), so `bounds_supported` says `False` for an expression carrying one even though `diff` now handles it
