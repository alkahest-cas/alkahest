...
Corpus: **294 observations** over **191 shape classes** — 89 certified, 2 conditional, 0 partial, 100 withheld.
...
71 shape classes — 41 certified, 30 withheld.
...
| ⛔ withheld | `form=product, funcs=exp, fn_arg=other, mul=general, pow=neg_one, pow_base=expr` | `((exp((-1 * x)))^-1 * C2)`, `((exp((-1/2 * x^2)))^-1 * C2)`, `((exp((-3 * x)))^-1 * C2)` | `collect_mul_factors`, `diff_exp`, `power_rule`, `product_rule` |
...
49 shape classes — 23 certified, 26 withheld.
