...
Corpus: **288 observations** over **187 shape classes** — 81 certified, 2 conditional, 0 partial, 104 withheld.
...
68 shape classes — 36 certified, 32 withheld.
...
| ⛔ withheld | `form=product, funcs=exp, fn_arg=other, mul=general, pow=neg_one, pow_base=expr` | `((exp((-1 * x)))^-1 * C2)`, `((exp((-1/2 * x^2)))^-1 * C2)`, `((exp((-3 * x)))^-1 * C2)` | `diff_exp`, `power_rule`, `product_rule` |
...
| ⛔ withheld | `form=product, funcs=exp, fn_arg=var, mul=general, pow=neg_one, pow_base=expr` | `((exp(x))^-1 * C2)` | `product_rule` |
...
| ⛔ withheld | `form=sum, funcs=sqrt, fn_arg=other, mul=general, pow=neg_one, pow_base=var` | `((-1 * sqrt((1 + x^2)) * x^-1) + 1)` | `diff_sqrt`, `product_rule`, `sum_rule` |

## `integrate`

48 shape classes — 20 certified, 28 withheld.
...
| ⛔ withheld | `form=sum, funcs=exp,log, fn_arg=var, mul=general, pow=neg_one, pow_base=var, definite=no` | `((exp(x) * log(x)) + (exp(x) * x^-1))` | — |
