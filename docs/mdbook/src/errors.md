...
| `E-NT-*` | `NumberTheoryError` | FLINT-backed integer number theory (`alkahest.number_theory`), including the classical arithmetic functions, which are exposed under `alkahest.experimental` — the partition function, Bernoulli, Euler, Stirling and harmonic numbers, Möbius μ, σ_k and sums of squares. `E-NT-006` is a **work cap**, not a claim that the value does not exist: `p(10^9)` is a perfectly good integer, this module simply will not spend unbounded time on a call that looked cheap. Note the Bernoulli convention is `B₁ = −1/2` |
