...
| `E-CHAR-*` | `CharacterError` | Conjugacy classes and **exact** character tables of permutation groups (`alkahest.experimental.ConjugacyClasses` / `CharacterTable`), by Dixon–Schneider. `E-CHAR-007` is the one that matters: a table that was computed, failed one of its **own** orthogonality checks, and was *withheld*. Row and column orthogonality, `Σ_i χ_i(1)² = |G|`, `χ_i(1) | |G|` and one character per class are all implied by the group axioms, so a violation is a defect in alkahest rather than bad input, and returning the table with a caveat is not an option — a caller cannot tell a checked table from an unchecked one once it is in their hands. `001` and `003` are two ceilings on *different* things: `001` caps `|G|`, because classes here are the orbits of `G` acting on itself and every element is held in memory (the group's order is still exact — it is the class partition that is refused, and nothing partial is returned), while `003` caps `φ(exp G)`, the degree of the one cyclotomic field every value lives in — the two are independent, so a group of small exponent passes `003` at any order and meets `001` instead, while the cyclic group of order 1000 is refused by `003` despite being small. The rest: `002` a cap above the hard ceiling, `004` a failed search for a prime `p ≡ 1 (mod exp G)` above `|G|` (a search-bound miss, not a mathematical obstruction), `005` an element outside the group, `006` an out-of-range class or character index, `008` the class multiplication matrices failing to split into one line per class, and `009` an internal invariant — reported rather than panicked because these run under a PyO3 boundary. A failure raised inside the permutation-group layer, the cyclotomic field or the GF(p) matrix layer keeps its own `E-GRP-*`, `E-NUMF-*` or `E-GFQ-*` code rather than being relabelled |
