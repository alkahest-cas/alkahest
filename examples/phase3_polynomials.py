...
p = ak.UniPoly.from_symbolic(expr, x)
...
# Powers
print(f"\n(x+1)^3 = {xp1 ** 3}")   # x^3+3*x^2+3*x+1
print(f"(x-1)^3 = {xm1 ** 3}")    # x^3-3*x^2+3*x-1
...
# MultiPoly displays the symbol names from the pool, with terms in the
# monomial order of the representation (not necessarily as written below).
...
a = ak.MultiPoly.from_symbolic(expr_a, [x2, y2])
...
b = ak.MultiPoly.from_symbolic(expr_b, [x2, y2])
print(f"\nb = 6x+4 = {b}")
print(f"  integer_content : {b.integer_content()}")  # 2

# Arithmetic
c = ak.MultiPoly.from_symbolic(pool2.add([x2, y2]), [x2, y2])
d = ak.MultiPoly.from_symbolic(
    pool2.add([x2, pool2.integer(-1)]), [x2, y2]
)
...
try:
    ak.UniPoly.from_symbolic(pool4.add([x4, y4]), x4)
