...
def build() -> dict:
    ...
    return {
        ...
        "corpus": {
            "sources": ["tests/lean_corpus.py", "tests/textbook_gate/"],
...
def render_markdown(ledger: dict) -> str:
    ...
    out.append('answer = ak.certifiable("integrate", ak.log(x), x)')
    out.append("bool(answer)      # False")
    out.append("answer.reason     # 'class_withheld'")
    out.append("")
    ...
    out.append("    ak.integrate(ak.log(x), x)   # raises E-CERT-001")
    out.append("```")
    out.append("")
    return "\n".join(out) + "\n"
