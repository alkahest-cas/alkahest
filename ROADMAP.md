...
| **2.0.5 – 3.10.0** | ✅ Complete | Thirty-odd releases this table never tracked, and it is not going to start: the per-release detail is in [CHANGELOG](CHANGELOG.md). Headline surfaces added over that span include validated numerics, SOS positivity certificates, creative telescoping (single-, `q`-, and `m`-index), the autoresearch modules (`ansatz`, `crosscheck`, `smt`, `research`), Puiseux series, the Cranelift tier, probability distributions with expectations and generating functions, information theory, and vector calculus with quaternions |
| **Next** | 🚧 In progress | CAD, generalized Pell, full algebraic Risch — see below |

**Test coverage:** 3341 Rust unit/proptest/doctest (`cargo test --workspace --features groebner`)
+ 3912 Python tests (`pytest tests/`, 105 skipped on a build without the optional
features and oracles), zero failures. Measured 2026-09-14 on `010b142`.
...
- **Native Rust codegen** — ✅ **Shipped** as the Cranelift tier (`--features cranelift`): CPU evaluation is interpreter → Cranelift → LLVM, selected by `CompileConfig`, so a prebuilt wheel gets native performance **without** shipping libLLVM. LLVM/NVPTX remains for GPU and MLIR interop.
