...
def _eval_or_nan(expr, bindings):
    """`eval_expr` raises (`DomainError`/`ValueError`) instead of returning a
    non-finite float for a point outside an expression's domain (e.g. a pole
    or a branch cut) — translate that back to `nan` so callers that already
    skip non-finite sample points via `math.isfinite` don't need to change.
    """
    try:
