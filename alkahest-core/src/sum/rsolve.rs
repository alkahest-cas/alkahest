...
fn rational_atom(pool: &ExprPool, r: &Rational) -> ExprId {
    let numer = Integer::from(r.numer().clone());
    let denom = Integer::from(r.denom().clone());
...
fn undetermined_order1(r: &Rational, h: &RatUniPoly) -> Option<RatUniPoly> {
    ...
    let start_deg = if *r == Rational::from(1) { 1 } else { 0 };
    for bump in 0..24 {
        let hi_deg = (dh + bump + if *r == Rational::from(1) { 1 } else { 0 }).max(start_deg);
        if hi_deg > 40 {
            break;
        }
...
fn sqrt_disc_expr(pool: &ExprPool, disc: &Rational) -> ExprId {
    ...
    let inner = pool.integer(Integer::from(prod));
    let sqrt_e = pool.func("sqrt", vec![inner]);
    let den_e = pool.integer(Integer::from(den));
...
fn peel_rational_root(p: &RatUniPoly) -> Option<Rational> {
    ...
    for c in &p.coeffs {
        lcm_den = lcm_den.lcm(&Integer::from(c.denom().clone()));
    }
    for c in &p.coeffs {
        let d = Integer::from(c.denom().clone());
        let scale = lcm_den.clone() / d;
        z.push(scale * Integer::from(c.numer().clone()));
    }
    ...
    try_vals.dedup();
    for r in try_vals {
        if !p.coeffs.is_empty() && horner_rat(p, &r) == Rational::from(0) {
            return Some(r);
        }
    }
    None
...
fn factor_char_polynomial(mut p: RatUniPoly) -> Result<Vec<(Rational, usize)>, RsolveError> {
    ...
    while p.degree() > 0 && guard < 64 {
        ...
        let mut m = 0usize;
        while p.degree() > 0 && horner_rat(&p, &r0) == Rational::from(0) {
...
fn hom_solution_from_roots(
    ...
    for (r, mult) in root_facts {
        ...
        for p in 0..*mult {
            ...
            let basis = if p == 0 {
                simp(pool, pool.pow(re.clone(), n_sym))
            } else {
...
fn apply_init(
    ...
    if d == 2 {
        ...
        let det = simp(
            pool,
            pool.add(vec![
                pool.mul(vec![a00.clone(), b10.clone()]),
                pool.mul(vec![a10.clone(), b00.clone(), pool.integer(-1_i32)]),
            ]),
        );
        let num_c0 = simp(
            pool,
            pool.add(vec![
                pool.mul(vec![v0p.clone(), b10.clone()]),
                pool.mul(vec![v1p.clone(), b00.clone(), pool.integer(-1_i32)]),
            ]),
        );
        let num_c1 = simp(
            pool,
        ...
        let c0v = expr_div(pool, num_c0, det.clone());
