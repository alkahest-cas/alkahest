...
mod tests {
    ...
    fn isolate_x_minus_1() {
        ...
        assert!(roots[0].lo <= rug::Rational::from(1));
        assert!(roots[0].hi >= rug::Rational::from(1));
    }
    ...
    fn real_roots_x_squared_minus_1() {
        ...
        assert!(roots[0].lo < rug::Rational::from(0));
        assert!(roots[1].lo >= rug::Rational::from(0));
    }
    ...
    fn real_roots_chebyshev_t4() {
        ...
        for r in &roots {
            assert!(r.lo >= rug::Rational::from(-1));
            assert!(r.hi <= rug::Rational::from(1));
        }
    }
    ...
    fn real_roots_symbolic_x_squared_minus_4() {
        ...
        assert_eq!(roots.len(), 2);
        assert!(roots[0].lo <= rug::Rational::from(-2));
        assert!(roots[0].hi >= rug::Rational::from(-2));
        assert!(roots[1].lo <= rug::Rational::from(2));
        assert!(roots[1].hi >= rug::Rational::from(2));
    }
