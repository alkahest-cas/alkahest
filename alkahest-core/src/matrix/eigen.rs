        ...
        match d {
            ...
            1 => {
                let root = linear_root(&base, pool)
                    .ok_or_else(|| EigenError::UnsupportedIrreducibleDegree { degree: d })?;
...
fn quadratic_roots(p: &UniPoly, pool: &ExprPool) -> Result<(ExprId, ExprId), EigenError> {
    ...
    if c1 == rug::Integer::from(0) {
        let r_rat = Rational::from((rug::Integer::from(0) - &c0, c2.clone()));
        let inner_sqrt = if r_rat.denom().clone() == rug::Integer::from(1) {
        ...
        match pool.get(e) {
            ExprData::Pow { base, exp } => {
                if let ExprData::Integer(n) = pool.get(exp) {
                    if n.0 == rug::Integer::from(2) && is_sqrt_of_negative_one(pool, base) {
...
fn expr_to_rational_strict(e: ExprId, pool: &ExprPool) -> Option<Rational> {
    match pool.get(e) {
        ...
        ExprData::Rational(ref r) => Some(r.0.clone().into()),
...
fn qi_nullspace_basis(
    ...
    for c in 0..cols {
        ...
        let mut piv = None;
        for rr in r..rows {
            if a[rr][c] != (Rational::from(0), Rational::from(0)) {
                piv = Some(rr);
                break;
            }
        }
        ...
        for rr in 0..rows {
            ...
            let f = a[rr][c].clone();
            if f == (Rational::from(0), Rational::from(0)) {
                continue;
            }
            for cc in 0..cols {
...
fn gauss_nullspace_expr(m: &Matrix, pool: &ExprPool) -> Result<Vec<Vec<ExprId>>, ()> {
    ...
    for c in 0..cols {
        ...
        for rr in 0..rows {
            ...
            for cc in 0..cols {
                let term = simplify(pool.mul(vec![f.clone(), a[r_at][cc]]), pool).value;
...
fn expr_is_exactly_zero(pool: &ExprPool, e: ExprId) -> bool {
    match pool.get(e) {
        ...
        ExprData::Rational(r) => r.0 == Rational::from(0),
        _ => false,
    }
}
