...
impl ArbBall {
    ...
    /// Lower bound of the interval, rounded **down** (`-∞` when the ball is
    /// indeterminate).
    ///
    ...
    ///
    /// An indeterminate ball (see `is_indeterminate`) is the whole real line,
    /// so its endpoints are `∓∞`: `mid - rad` on such a ball is NaN, and a NaN
    /// endpoint makes every comparison false, which reads as an enclosure that
    /// excludes the value it encloses.
    ...
    /// Upper bound of the interval, rounded **up** (`+∞` when the ball is
    /// indeterminate).  See [`ArbBall::lo`].
    ...
    /// Grow the radius outward by eight ulps of the ball's own **outer**
    /// magnitude, `(|mid| + rad) · 2^{-(prec-3)}`.
    ///
    /// This used to bump by `|mid| · 2^{-prec}`, which covers the rounding of
    /// `mid` and nothing else. The rules that build a ball from `f(lo)` and
    /// `f(hi)` accumulate several round-to-nearest steps — one on each
    /// endpoint, one on `(hi+lo)/2` and one on `(hi-lo)/2` — each worth half an
    /// ulp *of the endpoint*, not of the midpoint. Scaling by `|mid|` alone
    /// therefore under-covers whenever the ball is wide relative to its centre
...
impl ArbBall {
    ...
    pub fn powi(&self, n: i64) -> Self {
        ...
        ArbBall::from_endpoints(&min, &max, prec)
    }
    ...
    /// integer-valued exponent — goes to [`ArbBall::powi`], which encloses
    /// `xⁿ` from the interval's endpoints together with the `0` an even power
    /// attains in the interior, and whose reciprocal step returns `∞` exactly
    /// when the denominator straddles 0.
    ...
    /// Enclosure of `[lo, hi]`, rounding **outward**.  `lo <= hi` is the
    /// caller's business; a swapped pair yields the same (still enclosing)
    /// ball.  Every corner-evaluation rule in this module goes through here.
    ///
    /// `(lo+hi)/2` and `(hi-lo)/2` are each computed at `prec + 32` and rounded
    /// once, so building a ball from them could otherwise produce an enclosure
    /// *narrower* than the interval it was built from — by a sub-ulp margin,
    /// but a bound that is short by an ulp is not a bound.  A single
    /// [`Self::add_rounding_error`] covers both: it bumps by eight ulps of
    /// `|mid| + rad`, which dominates half an ulp of `mid` plus half an ulp of
    /// `rad` with room to spare.  (It is applied exactly once — two stacked
    /// bumps would only be wider, not more correct.)
    ///
    /// `sanitized` is the last step so that a non-finite endpoint becomes the
    /// whole real line rather than a ball with NaN endpoints, which `contains`
    /// would read as excluding every real number.
    fn from_endpoints(lo: &Float, hi: &Float, prec: u32) -> Self {
        let mid = Float::with_val(prec, Float::with_val(prec + 32, lo + hi) / 2u32);
        let rad = Float::with_val(prec, Float::with_val(prec + 32, hi - lo) / 2u32).abs();
        let mut b = ArbBall { mid, rad, prec };
        b.add_rounding_error();
        b.sanitized()
    }
