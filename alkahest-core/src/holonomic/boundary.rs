...
pub fn boundary_verdict(
    ...
    // the interior.
    //
    // `interior_poles` above already reports every pole it can *locate* as a
    // root of a denominator — including rational locations such as `k = (n+3)/2`
    // and poles of `G` rather than of `F` — and it runs first, so where both
    // apply the caller gets the location as well as the refusal. This adds the
    // two families it cannot see: a pole contributed by a `Γ(a·n + b·k + c)^{e>0}`
    // factor while `F`'s rational denominator is constant, and an offset further
    // from an endpoint than `affine_roots`' sampling grid reaches.
    if let Some(pt) = summand_pole_inside(&setup.f, setup.lo, setup.hi) {
        return BoundaryVerdict {
            status: BoundaryStatus::Unknown {
                reason: format!(
                    "the summand is undefined at k = {pt}, which lies inside the summation \
                     range k = {}..{}: that term of S(n) is a division by zero, so the sum \
                     has no value and no recurrence for it follows from the certificate",
                    setup.lo, setup.hi
                ),
            },
            valid_from,
            certificate_poles: vec![PolePoint {
                s: Rational::from(pt.alpha),
                t: Rational::from(pt.beta),
            }
            .to_expr(pool, n)],
        };
    }
